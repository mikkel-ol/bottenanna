...
async function play(message) {
	...
	// It's not a playlist
		...
		try {  // Play video with a clean URL
...
Select a song by returning a number on the list.
				...
				try {
					var response = await message.channel.awaitMessages(reply => reply.author == message.author && reply.content.split(" ")[0] > 0 && reply.content.split(" ")[0] < 11, {
						maxMatches: 1,
						time: 10000,
						errors: ['time']
					});
				...
				if (response[1]) {
					if (response[1].indexOf("t=") != -1) {
						var videoTimeStart = response[1].substr(response[1].indexOf("t=")+2);
					}
				}
		...
		return handleQueue(video, message, voiceChannel, false, videoTimeStart, repeat)
	}
}
...
function stop(message) {
	...
	message.delete();
	return message.channel.send(`❌ **${message.author.username}** stopped the fun :(`);
}
...
function skip(message) {
	...
	serverQueue.connection.dispatcher.end();
	message.delete();
	return message.channel.send(`⏩ **${message.author.username}** skipped song: **${song.title}**`);
}
...
function q(message) {
	const serverQueue = queue.get(message.guild.id);
	if (!serverQueue) return message.channel.send("Nothing is playing; no queue");
	if (serverQueue.songs.length == 1) return message.channel.send(`Nothing in queue, now playing: **${serverQueue.songs[0].title}**`);
...
${realQueue.map((song, i, arr) => {
	if (realQueue.length > 20 && arr.length - 1 === i) return `${song.title}`
...
function shuffle(message) {
	const serverQueue = queue.get(message.guild.id);
	if (!serverQueue) return message.channel.send("Can't shuffle an empty queue - nothing is playing!");
	if (serverQueue.songs.length == 1) return message.channel.send(`Nothing in queue, now playing: **${serverQueue.songs[0].title}**`);
...
async function handleQueue(video, msg, voiceChannel, playlist, videoTimeStart, repeat) {
...
function fading(message, diff, serverQueue, interval) {
	setTimeout(function() {
		...
		if (diff != 0) {
			...
			message.channel.send(`🔊 Volume is now: **${serverQueue.volume}%**`);
